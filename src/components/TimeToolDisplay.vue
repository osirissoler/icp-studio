<template>
  ...
  >
    ...
    <p v-if="tool.mode === 'clock' && tool.showDate" class="date-label">
      {{ dateLabel }}
    </p>
...
const displayScale = computed(() => {
  const value = Number(props.tool.displayScale ?? 1);
...
const hourHandStyle = computed(() => ({
  transform: `translateX(-50%) rotate(${
    (clockDate.value.getHours() % 12) * 30 + clockDate.value.getMinutes() / 2
  }deg)`,
}));

const minuteHandStyle = computed(() => ({
  transform: `translateX(-50%) rotate(${
    clockDate.value.getMinutes() * 6 + clockDate.value.getSeconds() / 10
  }deg)`,
}));
...
.digital-time {
  ...
  font-size: calc(clamp(52px, 12vw, 170px) * var(--time-display-scale, 1));
...
.time-stage--compact .digital-time {
  font-size: calc(clamp(28px, 7vw, 54px) * var(--time-display-scale, 1));
}
