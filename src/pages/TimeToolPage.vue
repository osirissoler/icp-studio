<template>
  <q-page class="time-tool-page">
    <header class="time-tool-page-header">
      <div class="time-tool-heading">
        <button
          type="button"
          class="back-button"
          aria-label="Volver"
        ...
        <span class="time-tool-heading-icon">
          <q-icon name="schedule" />
        </span>

        <div class="time-tool-heading-content">
          <span class="time-tool-eyebrow"> Herramientas · Control del tiempo </span>
      ...
      <aside class="time-settings panel-card">
        <div class="panel-title">
          <div>
            ...
            <strong>Prepara la herramienta</strong>
          </div>
        ...
        <div class="settings-scroll">
          ...
          <div class="display-size-setting">
            <div class="display-size-header">
              <label class="field-label"> Tamaño de los números </label>

              <strong> {{ Math.round(config.displayScale * 100) }}% </strong>
            </div>
            ...
            <q-slider
              ...
              color="primary"
              label
              :label-value="`${Math.round(config.displayScale * 100)}%`"
            />

            <div class="display-size-footer">
              <span>60%</span>
          ...
          <div class="separation-note">
            ...
            Esta vista sirve para preparar y probar. Sus controles no cambian lo que está en vivo.
          </div>
        </div>
      </aside>
      ...
      <q-card class="live-console">
        <header class="live-console-header">
          <div>
            ...
            <h2>
              {{ liveTimeTool?.title || 'Reloj y tiempo' }}
            </h2>
        ...
        <div class="live-console-body">
          ...
          <aside class="live-controls">
            ...
            <small>
              También puedes usar estos controles desde Contenido activo en la pantalla principal.
            </small>
          </aside>
        </div>
      </q-card>
...
function loadConfig(): TimeToolConfig {
  try {
    const saved = JSON.parse(localStorage.getItem(storageKey) ?? '{}') as Partial<TimeToolConfig>;
    ...
    return {
      ...
      displayScale:
        typeof saved.displayScale === 'number'
          ? Math.min(1.6, Math.max(0.6, saved.displayScale))
...
const config = reactive<TimeToolConfig>(loadConfig());
...
function startLocal(): void {
  ...
  if (config.mode === 'timer') {
    if (localFinishTimer !== null) {
      window.clearTimeout(localFinishTimer);
    }
...
function sendLive(): void {
  if (config.mode === 'timer' && config.durationMs <= 0) {
    return;
  }
...
.display-size-footer span:last-child {
  text-align: right;
}
